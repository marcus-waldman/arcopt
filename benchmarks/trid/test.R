# Test Trid Function Derivatives
# ================================
#
...
# Check gradient
cat("Gradient check:\n")
grad_check <- check_gradient(trid_fn, trid_gr, x1)
cat("  Max difference:", sprintf("%.2e", grad_check$max_diff), "\n")
cat("  Passed:        ", grad_check$passed, "\n\n")

# Check Hessian
cat("Hessian check:\n")
hess_check <- check_hessian(trid_gr, trid_hess, x1)
cat("  Max difference:", sprintf("%.2e", hess_check$max_diff), "\n")
cat("  Passed:        ", hess_check$passed, "\n\n")

cat("-"[rep(1, 60)], "\n\n", sep = "")

# Test point 2: At optimum for d=5
d <- 5
x2 <- sapply(1:d, function(i) i * (d + 1 - i))

cat("Test 2: At optimum for d=5\n")
cat("x =", x2, "\n")
cat("f(x) =", sprintf("%.4f", trid_fn(x2)))
cat(" (should be", -d * (d + 4) * (d - 1) / 6, ")\n\n")

grad_check2 <- check_gradient(trid_fn, trid_gr, x2)
cat("Gradient check:\n")
cat("  Max difference:", sprintf("%.2e", grad_check2$max_diff), "\n")
cat("  Passed:        ", grad_check2$passed, "\n\n")

hess_check2 <- check_hessian(trid_gr, trid_hess, x2)
cat("Hessian check:\n")
...
# Test point 3: At saddle point (origin)
x3 <- rep(0, 5)
cat("Test 3: At saddle point (origin)\n")
cat("x =", x3, "\n\n")

grad_check3 <- check_gradient(trid_fn, trid_gr, x3)
cat("Gradient check:\n")
cat("  Max difference:", sprintf("%.2e", grad_check3$max_diff), "\n")
cat("  Passed:        ", grad_check3$passed, "\n\n")

hess_check3 <- check_hessian(trid_gr, trid_hess, x3)
cat("Hessian check:\n")
