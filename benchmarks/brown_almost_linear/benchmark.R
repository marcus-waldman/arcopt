# Brown Almost-Linear benchmark
# =============================
#
# MGH #27. Near-rank-deficient Hessian at and near the solution due to
# the multiplicative term prod(x). Classical test where line-search
# quasi-Newton methods struggle with ill-conditioned / indefinite
# curvature.
#
# Usage (from repo root):
#   Rscript benchmarks/brown_almost_linear/benchmark.R [n] [kind] [n_seeds]
#
# kind: "standard" (x_0 = 0.5 vector) or "random"

suppressPackageStartupMessages({
  library(arcopt)
})

source(file.path("benchmarks", "brown_almost_linear", "setup.R"))

args <- commandArgs(trailingOnly = TRUE)
n       <- if (length(args) >= 1L) as.integer(args[[1]]) else 500L
kind    <- if (length(args) >= 2L) args[[2]]            else "standard"
n_seeds <- if (length(args) >= 3L) as.integer(args[[3]]) else 1L

maxit <- 2000L
gtol <- 1e-5

prob <- make_brown_problem(n = n)

cat(sprintf("Brown Almost-Linear: n = %d  start = %s  n_seeds = %d\n",
            n, kind, n_seeds))
...
for (seed in seq_len(n_seeds)) {
  if (kind == "standard") {
    x0 <- make_brown_start(n, kind = "standard")
  } else {
    x0 <- make_brown_start(n, kind = "random", seed = 100L + seed)
  }
  f0 <- prob$fn(x0)
...
cat("\n=============================================================\n")
cat(sprintf("Summary (converged = |g|_inf < %.0e AND f < 1e-6)\n", gtol))
cat("=============================================================\n")
...
out_dir <- file.path("benchmarks", "brown_almost_linear", "results")
