...
automatically adapts its subproblem solver to the local landscape:
...
2.  **Trust-region** (enabled by default). Fires a one-way switch from
    ...
    stuck-indefinite-saddle stagnation. Replaces the cubic penalty with
    a hard step-norm constraint so the step can walk off a near-singular
    ridge.
3.  **QN-polish** (opt-in). Fires a bidirectional switch from cubic when
    the iterate enters the quadratic attraction basin of a strict
...
See `design/design-principles.qmd` §3a for the σ↔︎r duality framing and
`todo/tr-fallback-hybrid-briefing.md` /
`todo/qn-polish-mode-briefing.md` for the detector designs.
